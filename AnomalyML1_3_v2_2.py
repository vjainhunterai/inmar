...
import urllib
import sys
import psutil  # MEMORY-OPT: for logging RSS memory at each stage
tqdm.pandas()
a=pd.DataFrame()
...
# MEMORY-OPT: Helper to log memory usage so we can see which step spikes.
# On 16GB RAM, anything above ~10GB RSS is danger zone.
def _mem(tag=""):
    try:
...
# MEMORY-OPT: Downcast numeric columns in-place to cut memory ~50% on numeric data.
# SAFE MODE: Only downcasts int64 -> smaller int; LEAVES FLOATS AS float64 to preserve
# full precision in amount comparisons like (Pay_Amount == Invoice_Amount) and
# the 1%/2% tolerance checks in rulebased8/rulebased10. If you want more RAM savings
# at the cost of ~1e-7 relative precision on floats, uncomment the float block.
def _downcast_numeric(df):
    # for c in df.select_dtypes(include=['float64']).columns:
    #     df[c] = pd.to_numeric(df[c], downcast='float')  # disabled for safety
...
def ML1():
    ...
    def readEncryptedConfig(excelFilePath,env):
        ...
        # Decrypt the data
        # MEMORY-OPT / COMPAT: applymap was deprecated in pandas 2.1 and removed in 2.2.
        # DataFrame.map is the replacement and is also slightly more memory-friendly.
        try:
            df_decrypted = df.map(lambda x: decryptData(str(x), keyPath))
        except AttributeError:
            # Fallback for pandas < 2.1
    ...
    def getDataFromDatabase():
        # MEMORY-OPT: Read the source table in chunks and downcast numerics
        # as each chunk arrives. This avoids holding one gigantic object-dtype
        # frame in RAM. On 16GB RAM, chunksize=100k is safe; drop to 50k if needed.
        a = readEncryptedConfig(r'/home/ubuntu/anomaly_data_pipeline/Data_Engg_Data_Science_Pre_Prod/Anomaly/Config/Paths.xls', env_local)
        user = a['user']
        pwd = a['password']
        password = urllib.parse.quote_plus(pwd)
        ...
        # MEMORY-OPT: chunked read + per-chunk downcast
        chunks = []
        with engine.connect() as conn:
    ...
    _mem("after_main_load")  # MEMORY-OPT: log memory after main source load
    ...
    # MEMORY-OPT: vendor_name is no longer needed after the merge — free it.
    del vendor_name
    gc.collect()
    ...
    # MEMORY-OPT: REMOVED `original_full_dataset_reserve_copy = combined.copy()`.
    # It was a full duplicate of the dataset held in RAM but never read back.
    # This alone saves ~1x the dataset size in memory.
    _mem("after_vendor_merge")
    ...
    print(combined.shape)
    print(datetime.now().strftime("%H:%M:%S"))
    # MEMORY-OPT: downcast again after all the type coercions above
    combined = _downcast_numeric(combined)
    _mem("after_all_filters")
    # MEMORY-OPT: REMOVED `combined_reserve_copy = combined.copy()`.
    # We now operate on `combined` directly for rulebased8 and keep ONE copy for
    # rulebased10. Previously we had combined + combined_reserve_copy + df(x2) in RAM.
    #uploadOutput_temorary(combined_reserve_copy, 'random1')
    gc.collect()
    # NOTE: original_full_dataset_reserve_copy was removed above, nothing to del here
    global group_id_counter
    gc.collect()
    ...
    try:
        # MEMORY-OPT: Keep ONE copy for rulebased10 BEFORE we mutate `combined`
        # in rulebased8. This replaces two separate .copy() calls that happened
        # inside each try-block previously.
        r10_source = combined.copy()
        _mem("after_r10_source_copy")

        df = combined  # MEMORY-OPT: view, not copy — we no longer need combined after r8
        print(datetime.now().strftime("%H:%M:%S"))
        ...
        # MEMORY-OPT: downcast Group_ID (rank returns float64 then cast)
        df['Group_ID'] = pd.to_numeric(df['Group_ID'], downcast='integer')

        group_id_counter = count(1)

        tqdm.pandas()

        # MEMORY-OPT: VECTORIZED rulebased8 logic.
        # The original did groupby('Group_ID').apply(check_group), which materializes
        # every group, runs Python code per group, and concatenates results — very
        # expensive in RAM and CPU with millions of groups.
        # We replace it with groupby transforms that operate on the whole frame at once.
        g = df.groupby('Group_ID', sort=False)

        grp_size = g['seq_no'].transform('size')
        # per-group: does any row have Pay_Amount == Invoice_Amount ?
        eq_mask = (df['Pay_Amount'] == df['Invoice_Amount'])
        has_match = eq_mask.groupby(df['Group_ID']).transform('any')
        # per-group: sum of Pay_Amount over rows where Pay != Invoice (filt_group)
        pay_filt = df['Pay_Amount'].where(~eq_mask, 0.0)
        total_pay = pay_filt.groupby(df['Group_ID']).transform('sum')
        # per-group: count of rows with Pay != Invoice (was len(filt_group) > 0)
        filt_count = (~eq_mask).groupby(df['Group_ID']).transform('sum')
        # per-group: max Invoice_Amount
        total_inv = g['Invoice_Amount'].transform('max')
        ...
        cond = (
            ...
            & (total_pay > 0)
            & (total_inv > 0)
            & ((total_inv - total_pay).abs() / total_inv.replace(0, np.nan) <= 0.01)
        )
        df['Flag'] = cond.fillna(False).astype(np.int8)  # int8, not int64

        # MEMORY-OPT: clean up intermediates before slicing
        del grp_size, eq_mask, has_match, pay_filt, total_pay, filt_count, total_inv, cond, g
        gc.collect()

        rulebased8 = df[df['Flag'] == 1].copy()
        ...
        _mem("after_rulebased8")

        # MEMORY-OPT: `combined` / `df` are the same object; we're done with both.
        del df, combined
        gc.collect()
    except Exception as e:
        import traceback
        traceback.print_exc()  # MEMORY-OPT / DEBUG: show full traceback, not just str(e)
        print(f"{e}")
    ...
    try:
        # MEMORY-OPT: use r10_source (prepared before rulebased8), no extra copy
        df = r10_source
        del r10_source  # MEMORY-OPT: drop the name so we don't double-reference
        print(datetime.now().strftime("%H:%M:%S"))
        pair_id_counter = count(1)
        df['Group_ID'] = (
         df[['Inv_Date','VENDOR_NAME_ALIAS',  'Invoice_Amount']]
        ...
        # MEMORY-OPT: Hard cap on combinations to prevent OOM.
        # 2^MAX_ITEMS grows fast — 15 = 32k combos (safe), 20 = 1M (risky), 25 = 33M (OOM).
        # LOGIC NOTE: Original code attempted combinations for any group size, wrapped
        # in a bare except that silently returned [] on MemoryError. So in practice
        # huge groups already produced Flag=0 (just via crash instead of skip). This
        # guard makes that behavior explicit and fast. We log how often it triggers.
        MAX_ITEMS_FOR_COMBINATIONS = 15
        _r10_skip_counter = {'skipped': 0, 'processed': 0}  # for post-run reporting

        def find_combinations(s_i, t_s):
            # MEMORY-OPT: guard against combinatorial explosion
            if len(s_i) > MAX_ITEMS_FOR_COMBINATIONS:
                return []
            result = []
        ...
        def check_group(group):
            ...
            if len(group)>0:
                # MEMORY-OPT: early skip of pathological groups — they produce no real
                # duplicate signal and their combinations explosion is what was killing RAM.
                if group['Suppliers_Invoice_Number'].nunique() > MAX_ITEMS_FOR_COMBINATIONS:
                    _r10_skip_counter['skipped'] += 1
                    group = group.copy()
                    group['Flag'] = 0
                    return group
                _r10_skip_counter['processed'] += 1
                seen1 = set()
        ...
        rulebased10=df[df['Flag']==1].copy()  # MEMORY-OPT: explicit copy before del df
        ...
        # LOGIC NOTE: report how many groups were skipped by the combinatorial guard.
        # If skipped > 0 and you want those groups processed, raise MAX_ITEMS_FOR_COMBINATIONS.
        print(f"rulebased10 combinatorial guard: processed={_r10_skip_counter['processed']}, "
              f"skipped(>{MAX_ITEMS_FOR_COMBINATIONS}items)={_r10_skip_counter['skipped']}")
        _mem("after_rulebased10")
        # MEMORY-OPT: release the big working frame before moving on
        del df
        gc.collect()
    except Exception as e:
        import traceback
        traceback.print_exc()  # MEMORY-OPT / DEBUG: show full traceback
        print(f"{e}")
    ...
    # MEMORY-OPT: free the two input frames now that we have the concatenated copy
    del rulebased8, rulebased10
    gc.collect()
    _mem("after_concat")
    rename_mapping = {
        'Chq_No': 'Check_Number',
    ...
    # MEMORY-OPT / BUGFIX: safe del — previously this would raise NameError if
    # the filtering block never set flag_check_number on this path.
    try:
    ...
    def getDataFromDatabase():
        # MEMORY-OPT: Original code did SELECT * FROM anomaly.duplicate_ap_invoice
        # just to get (a) the max Matched_Record_Number and (b) the column list.
        # On a large historical table this could easily be GBs. We now do two
        # tiny queries instead and return just what we need.
        a = readEncryptedConfig(r'/home/ubuntu/anomaly_data_pipeline/Data_Engg_Data_Science_Pre_Prod/Anomaly/Config/Paths.xls', env_local)
        user = a['user']
        pwd = a['password']
        password = urllib.parse.quote_plus(pwd)
        ...
        schema = "anomaly"
        engine = create_engine('mysql+mysqlconnector://{0}:{1}@{2}/{3}'.format(user, password, ip, schema))
        # (1) column names only — LIMIT 0 returns no rows but full schema
        cols_query = "SELECT * FROM anomaly.duplicate_ap_invoice LIMIT 0"
        # (2) max Matched_Record_Number — scalar, cheap
        max_query = "SELECT MAX(Matched_Record_Number) AS max_mrn FROM anomaly.duplicate_ap_invoice"
        with engine.connect() as conn:
            cols_df = pd.read_sql(cols_query, conn)
            max_df = pd.read_sql(max_query, conn)
        # Build a lightweight frame that mimics what the original returned: empty
        # rows but correct columns, PLUS one row carrying the max value so the
        # downstream code path (len(combined1)>0 and combined1['Matched_Record_Number'].max())
        # still works without loading the whole table.
        result = cols_df.copy()
        max_val = max_df.iloc[0, 0] if len(max_df) else 0
        if pd.notna(max_val):
            # seed one sentinel row with just the max; other cols stay NaN/None
            seed = {c: None for c in result.columns}
            if 'Matched_Record_Number' in seed:
                seed['Matched_Record_Number'] = max_val
            result = pd.concat([result, pd.DataFrame([seed])], ignore_index=True)
        return result
    ...
    # MEMORY-OPT: combined1 was only needed for its column list and max_mrn; free it.
    del combined1
    gc.collect()
    _mem("before_upload")
    print(combined.shape)
    def uploadOutput(output):
        ...
        # MEMORY-OPT: chunksize on to_sql avoids the connector buffering the full
        # INSERT statement in memory; method='multi' batches rows per INSERT.
        output.to_sql(table_name, con=engine, if_exists='append', index=False,
                      chunksize=5000, method='multi')

    uploadOutput(combined)
    # MEMORY-OPT: release the final frame and log peak
    del combined
    gc.collect()
    _mem("after_upload")
